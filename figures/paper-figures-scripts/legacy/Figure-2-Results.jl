...
using PyCall, PyPlot, CairoMakie, GLMakie, LaTeXStrings
...
# Functions
function plot_material(np, pv, plotter, design, W, L; colorbar=true, title="\$\\rho(x)\$", font_size=20, title_font_size=24, num_periods_x=1, num_periods_y=1, ontop=false, design_field="p", contour=true, full=false, clim=(0.0, 1.0), opacity=nothing, reflectybool=true, flipx=false, flipy=true, color="#b097d1")
    ...
    scalargs = if ontop
        Dict("title" => title, "vertical" => false, "label_font_size" => font_size, "title_font_size" => title_font_size, "position_x" => 0.25, "position_y" => 0.8, "color" => "Black", "use_opacity" => false)
    else
        Dict("title" => title, "vertical" => true, "label_font_size" => font_size, "title_font_size" => title_font_size, "position_x" => 0.0, "position_y" => 0.35, "color" => "Black", "use_opacity" => false)
    end

    for i in 0:1:num_periods_x-1
        for j in 0:1:num_periods_y-1
            ps = Vector(0:0.01:1.0)
            current_opacity = (isnothing(opacity)) ? (x -> ifelse(x < 1.0, 0.0, x / 20)).(ps) : opacity
            plotter.add_mesh(design.translate((i * W * 2, j * L * 2, 0), inplace=false), color=contour_color, clim=clim, opacity=current_opacity, show_scalar_bar=(colorbar & (i == j) & (j == 0)), scalar_bar_args=scalargs)
            if !full
                plotter.add_mesh(reflectx.translate((i * W * 2, j * L * 2, 0), inplace=false), color=contour_color, clim=clim, show_scalar_bar=false, opacity=current_opacity)
                if reflectybool
                    plotter.add_mesh(reflecty.translate((i * W * 2, j * L * 2, 0), inplace=false), color=contour_color, clim=clim, show_scalar_bar=false, opacity=current_opacity)
                    plotter.add_mesh(reflectxy.translate((i * W * 2, j * L * 2, 0), inplace=false), color=contour_color, clim=clim, show_scalar_bar=false, opacity=current_opacity)
                end
            end
        end
    end
...
final_figure_filename = "figures/Figure-2-Results.pdf"

# --- 1. Load Data ---
println("Loading spectral data...")
spec_con_mono = get_data(constrained_group, mono_opt_variation, "spectral")
spec_con_bi = get_data(constrained_group, bi_opt_variation, "spectral")
spec_sphere = get_data(benchmark_group, benchmark_variation, "spectral")
println("Loading geometry data...")
mesh_con_mono = get_data(constrained_group, mono_opt_variation, "design_y")
mesh_con_bi = get_data(constrained_group, bi_opt_variation, "design_y")
mesh_sphere = get_data(benchmark_group, benchmark_variation, "design_y")

if any(isnothing, [spec_con_mono, spec_con_bi, spec_sphere, mesh_con_mono, mesh_con_bi, mesh_sphere]) || !PYVISTA_AVAILABLE
    error("Failed to load required data or PyVista is not available.")
end
...
function plot_geometry_for_inset(mesh_pyobject, output_filename; scalar_field="p", window_size=(400, 400), color_hex_str::String, flipy=false)
    ...
    plot_material(np, pv, plotter, mesh_pyobject, W, L; colorbar=false, num_periods_x=2, num_periods_y=2, contour=true, design_field=scalar_field, color=color_hex_str, flipy=flipy)
