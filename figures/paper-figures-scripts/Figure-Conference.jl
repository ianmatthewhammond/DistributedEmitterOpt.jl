...
using Images, FileIO, Colors, ColorSchemes, JLD2
...
if !@isdefined(con_data)
    ...
    # import Emitter3DTopOpt as e3 # Removed

    global uns_data, con_data = Visualization.load_bid_parameters(pv, np, path_bid, path_mono)
else
...
end#         plotter.add_mesh(water.translate((i * W * 2, j * L * 2, 0), inplace=false), color="b", opacity=0.001)
#         plotter.add_mesh(reflectxwater.translate((i * W * 2, j * L * 2, 0), inplace=false), color="b", opacity=0.001)
#         plotter.add_mesh(reflectywater.translate((i * W * 2, j * L * 2, 0), inplace=false), color="b", opacity=0.001)
#         plotter.add_mesh(reflectxywater.translate((i * W * 2, j * L * 2, 0), inplace=false), color="b", opacity=0.001)
#     end
...
# # Using plot_material equivalent logic or calling it if compatible
# # Fig 1 calls `plot_material` with specific args.
# plot_material(np, pv, plotter, design, W, L;
#     colorbar=false, full=false, flipx=flip1x, flipy=flip1y,
#     num_periods_x=2, num_periods_y=2, color="#b097d1")

Visualization.plot_substrate(np, pv, plotter, y_fields, W, L, des_low; full=false, flipx=flip1x, flipy=flip1y, num_periods_x=2, num_periods_y=2)
...
plotter.close()
crop_white_margins(overview_temp_path, overview_temp_path)
println("Overview image saved.")

# Define get_data helper
function get_data(category, subcategory, type)
    base_path = joinpath(figures_dir, "data", category, subcategory)
    if type == "spectral"
        res = Visualization.get_spectral_data(base_path)
        return Dict(
            "wavelengths" => res[1],
            "g_y" => res[2],
            "g_x" => res[3],
    ...
    elseif type == "design_y"
        return pv.read(joinpath(base_path, "y_design.vtu"))
    end
end
...
# Metal Mono (Purple)
plot_geometry_for_inset(np, pv, mesh_metal_nominal, inset_metal_mono_path;
    color=COLOR_METAL_ISOTROPIC, flipy=true, window_size=(300, 300), scalar_field="p")
crop_white_margins(inset_metal_mono_path, inset_metal_mono_path; threshold=0.99)

# Metal Bi (Blue/Darker)
const COLOR_BIDIRECTIONAL = "#496cb7"
plot_geometry_for_inset(np, pv, mesh_metal_bi, inset_metal_bi_path;
    color=COLOR_BIDIRECTIONAL, flipy=true, window_size=(300, 300), scalar_field="p")
crop_white_margins(inset_metal_bi_path, inset_metal_bi_path; threshold=0.99)

# Dielectric (Red)
plot_geometry_for_inset(np, pv, mesh_diel_nominal, inset_diel_path;
    color=COLOR_DIEL_ISOTROPIC, flipy=true, window_size=(300, 300), scalar_field="p")
crop_white_margins(inset_diel_path, inset_diel_path; threshold=0.99)

# Sphere (Green)
plot_geometry_for_inset(np, pv, mesh_sphere, inset_sphere_path;
    color="green", flipy=false, window_size=(300, 300), scalar_field="p")
crop_white_margins(inset_sphere_path, inset_sphere_path; threshold=0.99)
...
lines!(ax_results, spec_diel_nominal["wavelengths"], spec_diel_nominal["g_y"],
    color=COLOR_DIEL_ISOTROPIC, linewidth=5)
...
scatter!(ax_results, [spec_diel_nominal["wavelengths"][idx_peak_diel]], [spec_diel_nominal["g_y"][idx_peak_diel]],
    color=:white, markersize=18, marker=:circle, strokewidth=3, strokecolor=COLOR_DIEL_ISOTROPIC)
# REFINEMENT: Move "Dielectric" to right side, above inset, other side of lambda_p line (532).
# Try 535nm.
text!(ax_results, 500.0, 10^1.75,
    text="Dielectric (elastic)", color=COLOR_DIEL_ISOTROPIC, fontsize=30, align=(:left, :center))
...
lines!(ax_results, spec_metal_aniso["wavelengths"], spec_metal_aniso["g_y"],
    color=COLOR_METAL_ISOTROPIC, linestyle=:solid, linewidth=5)
...
scatter!(ax_results, [spec_metal_aniso["wavelengths"][idx_peak_ani]], [spec_metal_aniso["g_y"][idx_peak_ani]],
    color=:white, markersize=18, marker=:circle, strokewidth=3, strokecolor=COLOR_METAL_ISOTROPIC)
...
text!(ax_results, 552.0, spec_metal_aniso["g_y"][idx_peak_ani] * 1.6,
    text="Metal (elastic)", color=COLOR_METAL_ISOTROPIC, fontsize=30, align=(:left, :center))

# 4. Metal Inelastic (Fig 4b) - Green/Grey Dotted
lines!(ax_results, spec_metal_inelastic["wavelengths"], spec_metal_inelastic["g_y"],
    color=COLOR_METAL_INELASTIC, linestyle=:dot, linewidth=5)
...
scatter!(ax_results, [spec_metal_inelastic["wavelengths"][idx_peak_inel]], [spec_metal_inelastic["g_y"][idx_peak_inel]],
    color=:white, markersize=18, marker=:circle, strokewidth=3, strokecolor=COLOR_METAL_INELASTIC)
text!(ax_results, 525.0, 10^3.6,
    text="Metal (inelastic)", color=COLOR_METAL_INELASTIC, fontsize=30, align=(:center, :bottom)) # Font 30
...
vlines!(ax_results, [532], color=COLOR_PUMP, linestyle=:dash, linewidth=2)
text!(ax_results, 532 + 0.5, 10^0.9, text=L"\textrm{pump}\;\lambda_p=532\textrm{nm}", color=COLOR_PUMP, align=(:left, :center), fontsize=30) # Font 30
text!(ax_results, 532 + 0.5, 10^0.7, text=L"\textrm{elastic emission}\;\lambda_e", color=COLOR_PUMP, align=(:left, :center), fontsize=30) # Font 30

# Vertical Line for Emission
vlines!(ax_results, [549], color=COLOR_EMISSION, linestyle=:dash, linewidth=2)
text!(ax_results, 549 + 0.5, 10^0.9, text=L"\textrm{inelastic emission}\;\lambda_e", color=COLOR_EMISSION, align=(:left, :center), fontsize=30) # Font 30
text!(ax_results, 549 + 0.5, 10^0.7, text=L"=549\textrm{nm}", color=COLOR_EMISSION, align=(:left, :center), fontsize=30) # Font 30
...
# Save
final_pdf_path = joinpath(@__DIR__, "figures/Figure-Metamaterials.pdf")
